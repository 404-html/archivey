...
cursor = conn.cursor()
cursor.execute('create table if not exists messages (id text, message text, user text, channel text, timestamp text, UNIQUE(id) ON CONFLICT REPLACE)')
...
with open(os.path.join(directory, 'channels.json'),'r',encoding='utf-8') as f:
